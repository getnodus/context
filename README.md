...
| --- | --- | --- | --- |
| Update check | Requests `https://registry.npmjs.org/@getnodus/context/latest` to compare your installed version to the latest published version. 1.5s timeout; failures are silent. | At most once every 24 hours when the cache is stale — on CLI startup, on `context doctor` / `context update`, and once when the MCP server starts. The cached result is reused (no network) elsewhere, e.g. the MCP brief. Skipped automatically in CI and dev builds. | Set `NODUS_DISABLE_UPDATE_CHECK=1`. |
| Verify blocks | For `verify:` specs, `kind: url` fetches the URL you specified and `kind: repo` checks `https://api.github.com/repos/<owner>/<name>`. `kind: path` is local-only. | Inline when a write performs verification, and when you explicitly run verify. Background re-check fires on read once an entry's `verify:` is older than 7 days — enabled in the MCP server, off by default for library/CLI callers. | Set `NODUS_DISABLE_BACKGROUND_VERIFY=1` to suppress stale-on-read background checks. Set `NODUS_VERIFY_TIMEOUT_MS` to change the normal timeout (8s default); inline verify-on-write still caps at 3s. |
