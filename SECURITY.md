...
- **Bearer tokens** for the `http` and `mirror` backends, stored in `~/.nodus/config.json` (or `$NODUS_CONFIG_DIR/config.json`) under `profiles.<name>.token`. The file is written with mode `600` when saved via the CLI.
- **CLI JSON output** — `context profile list --json` and `context config show --json` redact bearer tokens to `<redacted>`. Read `~/.nodus/config.json` directly when you need the raw token.
...
- **Verify blocks** — outbound requests to URLs/repos declared in entries; URL verifies block private/internal hosts, resolve DNS before fetching, and validate each redirect hop. Disable with `NODUS_DISABLE_BACKGROUND_VERIFY=1`.
